...
const formatRangeLabel = (startIso: string, endIso: string) => {
  ...
  return `${datePart} ò ${formatTime(startIso)} û ${formatTime(endIso)}`;
};
...
export default function PlanningRoomsPage({
  ...
  const loadRoom = React.useCallback(
    async (roomId: string) => {
      ...
      try {
        ...
        console.error(error);
        const message = error instanceof Error ? error.message : "Error desconocido al cargar la sala";
  ...
  const onSaveRoom = async () => {
    ...
    try {
      ...
      console.error(error);
      const message = error instanceof Error ? error.message : "Error al guardar la sala";
      setFeedback({ type: "error", message });
    } finally {
      setSaving(false);
    }
  ...
  const onCreateRoom = async () => {
    ...
    try {
      ...
      console.error(error);
      const message = error instanceof Error ? error.message : "Error al crear la sala";
  ...
  const onAddBlock = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/rooms/${selectedRoomId}/blocks`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          startsAt: startIso,
      ...
      console.error(error);
      const message = error instanceof Error ? error.message : "Error al crear el bloqueo";
      setFeedback({ type: "error", message });
    } finally {
  ...
  const onRemoveBlock = async (blockId: string) => {
    ...
    setFeedback(null);
    try {
      const response = await fetch(`/api/rooms/${selectedRoomId}/blocks?blockId=${blockId}`, {
        method: "DELETE",
      });
      if (!response.ok) {
        const data = await response.json().catch(() => null);
      ...
      console.error(error);
      const message = error instanceof Error ? error.message : "Error al eliminar el bloqueo";
      setFeedback({ type: "error", message });
    } finally {
  ...
  const onSaveRecurring = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/rooms/${selectedRoomId}/recurring-blocks`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      console.error(error);
      const message = error instanceof Error ? error.message : "Error al guardar bloqueos recurrentes";
      setFeedback({ type: "error", message });
    } finally {
                      ...
                      <li key={room.id}>
                        ...
                        >
                          ...
                          <span className="text-xs text-slate-500">
                            ...
                            {supportsLocation && room.location ? ` ò ${room.location}` : ""}
                          </span>
                        </button>
                      </li>
                    );
