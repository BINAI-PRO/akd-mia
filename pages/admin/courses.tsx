...
export const getServerSideProps: GetServerSideProps<PageProps> = async () => {
  ...
  if (coursesResp.error) {
    throw coursesResp.error;
  }
  if (instructorsResp.error) {
    throw instructorsResp.error;
  }
  if (classTypesResp.error) {
    throw classTypesResp.error;
  }

  const courseRows = (coursesResp.data ?? []) as CourseQueryRow[];
  const instructorRows = (instructorsResp.data ?? []) as Tables<'instructors'>[];
  const classTypeRows = (classTypesResp.data ?? []) as Tables<'class_types'>[];

  const initialCourses = courseRows.map(mapCourse);
  const instructors = instructorRows.map(({ id, full_name }) => ({ id, full_name }));
  ...
  courseRows.forEach((row) => {
    if (row.category && row.category.trim()) categorySet.add(row.category.trim());
  });
  if (!categorySet.has('General')) categorySet.add('General');

  const levelOptions = Array.from(levelSet).sort((a, b) => a.localeCompare(b, 'es'));
  const categoryOptions = Array.from(categorySet).sort((a, b) => a.localeCompare(b, 'es'));

  return { props: { initialCourses, instructors, classTypes, levelOptions, categoryOptions } };
};
...
export default function CoursesPage(
  ...
  const { initialCourses, instructors, classTypes, levelOptions, categoryOptions } = props;
  ...
  const resetForm = (options?: { nextLevels?: string[]; nextCategories?: string[] }) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!formState.classTypeId) {
        throw new Error('Selecciona un tipo de curso');
      }
      ...
      const body = (await response.json().catch(() => ({}))) as Partial<CourseApiResponse & { error?: string }>;
      if (!response.ok) {
      ...
      setFormError(error instanceof Error ? error.message : 'No se pudo crear el curso');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="mx-auto flex max-w-6xl gap-6">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative">
                <span className="material-icons-outlined pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">search</span>
                <input
                  type="search"
                  value={searchTerm}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          {course.coverImageUrl ? (
                            // eslint-disable-next-line @next/next/no-img -- admin list allows remote cover preview
                            <img
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-500">
                              {[course.classTypeName, course.Nivel, course.Categoria ?? 'General']
                                .filter(Boolean)
                                .join(' | ')}
                              {course.leadInstructorName ? ` | ${course.leadInstructorName}` : ''}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-slate-700">
                        {course.sessionCount} sesiones x {course.sessionDurationMinutes} min
                      </td>
                      <td className="px-4 py-3 text-slate-700">
                        {course.Precio !== null
        ...
        <div ref={formRef} className="w-full max-w-[360px] shrink-0">
          <div className="rounded-lg border border-slate-200 bg-white p-4">
            <h2 className="mb-3 text-base font-semibold text-slate-800">Crear curso</h2>
            {!hasClassTypeOptions && (
            ...
            <form className="space-y-4 text-sm" onSubmit={handleSubmit}>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700">Descripcion corta</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700">Tipo de curso</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700">Instructor principal</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-slate-700">Sesiones totales</label>
                  <input
                    type="number"
                    min={1}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700">Minutos por sesión</label>
                  <input
                    type="number"
                    min={10}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700">Etiqueta de duración</label>
                <input
