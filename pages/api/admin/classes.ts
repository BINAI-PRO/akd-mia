...
export default async function handler(
  ...
  if (req.method === 'PATCH') {
    try {
      ...
      if (!sessionId) {
        return res.status(400).json({ error: 'Debes indicar la clase a actualizar' });
      }
      ...
      if (fetchError || !current) {
        console.error('/api/admin/classes PATCH lookup', fetchError);
      ...
      if (Object.keys(updates).length === 0) {
        ...
        if (snapshotError || !sessionSnapshot) {
          console.error('/api/admin/classes PATCH fetch', snapshotError);
          return res.status(500).json({ error: 'No se pudo obtener la clase actualizada' });
        }
        return res.status(200).json({ message: 'Clase actualizada', session: sessionSnapshot });
      }
      ...
      if (updateError || !updatedSession) {
        console.error('/api/admin/classes PATCH update', updateError);
        return res.status(500).json({ error: 'No se pudo actualizar la clase' });
      }

      return res.status(200).json({ message: 'Clase actualizada', session: updatedSession });
    } catch (error) {
      console.error('/api/admin/classes PATCH', error);
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (courseError || !course) {
        console.error('/api/admin/classes POST course lookup', courseError);
        return res.status(400).json({ error: 'El curso seleccionado no existe' });
      }

      if (!course.class_type_id) {
        return res.status(400).json({ error: 'El curso seleccionado no tiene un tipo de clase configurado' });
      }
      ...
      console.error('/api/admin/classes POST', error);
      const message = error instanceof Error ? error.message : 'Error inesperado';
      return res.status(500).json({ error: message });
    }
  }
