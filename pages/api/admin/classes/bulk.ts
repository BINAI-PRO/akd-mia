...
export default async function handler(
  ...
  try {
    ...
    if (!action || !Array.isArray(sessionIds) || sessionIds.length === 0) {
      return res.status(400).json({ error: 'Debes seleccionar clases para aplicar la accion' });
    }
    ...
    if (lookupError) {
      console.error('/api/admin/classes/bulk lookup', lookupError);
      return res.status(500).json({ error: 'No se pudieron validar las clases seleccionadas' });
    }
    if (!sessions || sessions.length !== sessionIds.length) {
      return res.status(404).json({ error: 'No se encontraron todas las clases seleccionadas' });
    }

    if (action === 'reschedule') {
      const invalid = sessions.filter((row) => (row.current_occupancy ?? 0) > 0);
      if (invalid.length > 0) {
        return res.status(400).json({ error: 'No puedes reprogramar clases con reservaciones activas' });
      }
      ...
      if (deleteError) {
        console.error('/api/admin/classes/bulk delete', deleteError);
        return res.status(500).json({ error: 'No se pudieron reprogramar las clases seleccionadas' });
      }

      return res.status(200).json({
        message: 'Clases enviadas a reprogramacion',
    ...
    if (action === 'update') {
      ...
      if (updateError || !updatedSessions) {
        console.error('/api/admin/classes/bulk update', updateError);
        return res.status(500).json({ error: 'No se pudieron actualizar las clases' });
      }

      return res.status(200).json({ message: 'Clases actualizadas', sessions: updatedSessions });
    }

    return res.status(400).json({ error: 'Accion no valida' });
  } catch (error) {
    console.error('/api/admin/classes/bulk', error);
