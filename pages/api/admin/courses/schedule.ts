...
export default async function handler(
  ...
  if (courseError || !course) {
    console.error("/api/admin/courses/schedule course lookup", courseError);
    return res.status(400).json({ error: "El curso seleccionado no existe" });
  }

  if (!course.class_type_id) {
    return res.status(400).json({ error: "El curso no tiene un tipo de clase asignado" });
  }
  ...
  if (countError) {
    console.error("/api/admin/courses/schedule count", countError);
  ...
  if (insertError || !inserted) {
    console.error("/api/admin/courses/schedule insert", insertError);
